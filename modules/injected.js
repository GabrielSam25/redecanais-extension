...
(function() {
    ...
    window.VideoJSDebugger = class {
            ...
            Object.keys(debugEvents).forEach(event => {
                player.on(event, function() {
                    ...
                    if (event === 'play' || event === 'pause') {
                        ...
                        this.sendNotification('🏁 Vídeo finalizado - Pronto para próximo episódio!', 'success');
                    }
        ...
        setupEnhancedMonitoring() {
            window.addEventListener('message', (event) => {
                if (event.data.type === 'VIDEOJS_DEBUG_MONITOR_PLAYER') {
                    const playerId = event.data.playerId;
                    const events = event.data.events || [];
                    
                    if (window._videoDebugPlayers && window._videoDebugPlayers[playerId]) {
                        const player = window._videoDebugPlayers[playerId];
    ...
    window.initializeVideoJSDebugger = function() {
        return new VideoJSDebugger();
    };

    console.log('✅ VideoJSDebugger carregado e pronto');
})();